...
GENERATED_SCRIPT = r'''#!/usr/bin/env python3
from __future__ import annotations
...
PARADIGM = __PARADIGM_JSON__
EXPERIMENT_PLAN = __EXPERIMENT_PLAN_JSON__
...
def _script_analysis_code(paradigm: dict[str, Any], experiment_plan: dict[str, Any]) -> str:
    code = GENERATED_SCRIPT.replace("__PARADIGM_JSON__", pformat(paradigm, width=100, sort_dicts=False))
    code = code.replace("__EXPERIMENT_PLAN_JSON__", pformat(experiment_plan, width=100, sort_dicts=False))
    code = code.replace("#!/usr/bin/env python3\n", "")
    cli_marker = "\n\ndef run(input_root: Path, output_dir: Path, subject: str | None, session: str | None) -> dict[str, Any]:\n"
    if cli_marker in code:
        code = code.split(cli_marker, 1)[0].rstrip() + "\n"
    return code


def _analysis_code(paradigm: dict[str, Any], experiment_plan: dict[str, Any]) -> str:
    return _script_analysis_code(paradigm, experiment_plan)


def _class_based_pipeline_code() -> str:
    return r'''
import argparse


class ExperimentContext:
    """保存一次实验执行的路径、被试、实验路线和中间结果。"""
    ...
    def plan_path(self) -> Path:
        return self.output_dir / f"{PARADIGM['id']}_class_pipeline_plan.json"
...
class ExperimentPlanStep:
    """步骤 2：写出中文实验计划，便于追溯创新点和实验路线绑定关系。"""
...
class ParadigmRunnerStep:
    ...
    def record_runner_message(self, context: ExperimentContext) -> None:
        context.add_message(f"已执行实验路线 runner: {PARADIGM['id']}，状态={context.result.get('status')}")
...
class ConnectomeValidationStep:
    """步骤 4：功能连接实验路线必须生成 3D connectome HTML 和俯视图。"""
    ...
    def run(self, context: ExperimentContext) -> ExperimentContext:
        if not self.requires_connectome():
            context.add_message("当前实验路线不需要 3D connectome 强制校验。")
            return context
...
def write_class_based_python_script(path: Path, paradigm: dict[str, Any], experiment_plan: dict[str, Any]) -> None:
    code = _analysis_code(paradigm, experiment_plan).rstrip()
    code = f"{code}\n\n{_class_based_pipeline_code().lstrip()}"
    path.write_text(code, encoding="utf-8")


def generate_experiment_code(paradigm: dict[str, Any], out_dir: Path, linked_innovations: list[dict[str, Any]] | None = None) -> Path:
    out_dir.mkdir(parents=True, exist_ok=True)
    compact_innovations = []
    for item in linked_innovations or []:
        compact_innovations.append(
            {
                "id": item.get("id", ""),
                ...
                "testable_outputs": item.get("testable_outputs", []),
                "risk_factors": item.get("risk_factors", []),
            }
        )
    experiment_plan = {
        "说明": "实验代码生成计划：从创新点和假设出发，明确可测量输出、分析步骤、风险控制，并在执行后用于反向更新创新点。",
        "paradigm_id": paradigm.get("id", ""),
        "paradigm_name": paradigm.get("name", ""),
        "linked_innovations": compact_innovations,
        "analysis_goal_zh": f"用 {paradigm.get('name', paradigm.get('id', 'selected paradigm'))} 的可执行输出检验关联创新点中的神经科学假设。",
        "measurable_outputs": paradigm.get("outputs", []),
        "required_data": paradigm.get("required_data", []),
        "missing_data": paradigm.get("missing_data", []),
        "analysis_steps_zh": [
            "检查输入目录中的被试、session 和可用数据类型。",
            "运行该 paradigm 对应的分析函数，生成结构化 result.json。",
            "提取关键指标、输出图表或中间表格。",
            "记录失败原因、缺失数据和解释边界，供创新点反向更新使用。",
        ],
        "risk_controls_zh": [
            "如果缺失必要数据，则将结果标记为 not_executable，而不是强行得出结论。",
            "如果分析脚本失败或超时，后续创新点更新只解释可执行性风险，不把失败当作科学反证。",
            "所有 result.json 中的指标都作为后续 LLM 反向更新的证据来源。",
        ],
    }
    plan_path = out_dir / f"{paradigm['id']}_class_pipeline_plan.json"
    plan_path.write_text(json.dumps(experiment_plan, ensure_ascii=False, indent=2), encoding="utf-8")
    script_path = out_dir / f"{paradigm['id']}.py"
    write_class_based_python_script(script_path, paradigm, experiment_plan)
    return script_path
