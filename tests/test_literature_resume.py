...
def test_long_run_resume_from_checkpoint(tmp_path):
    checkpoint = write_checkpoint(tmp_path, {"run_id": "r1", "run_dir": str(tmp_path), "status": "ok"}, iteration=1)
    result = run_long_run("configs/literature_long_run_config.yaml", resume=checkpoint, dry_run=True, backend="postgresql")
