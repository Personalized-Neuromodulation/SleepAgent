...
def _no_real_foundation_config(tmp_path: Path) -> Path:
    config = load_config("configs/foundation_no_real_data.yaml")
    ...
    config["outputs"].update(
        {
            ...
            "report": str(tmp_path / "reports" / "data_foundation_report.md"),
        }
    )
    return _write_yaml(tmp_path / "foundation_no_real_data.yaml", config)


def _no_fixture_grounding_config(tmp_path: Path) -> Path:
    ...
    return _write_yaml(tmp_path / "grounding_no_fixtures.yaml", config)


def test_no_real_data_foundation_writes_empty_declared_basis(tmp_path):
    result = run_foundation_pipeline(_no_real_foundation_config(tmp_path))

    assert result["mode"] == "no_real_data"
...
def test_grounding_without_real_features_builds_evidence_and_theoretical_graph(monkeypatch, tmp_path):
    run_foundation_pipeline(_no_real_foundation_config(tmp_path))
    monkeypatch.setattr("sleep_ai_scientist.grounding.grounding_pipeline.search_literature_apis", fake_online_literature_search)
    result = run_grounding_pipeline(_no_fixture_grounding_config(tmp_path))
