...
class EEGFeatureAgent:
    ...
    def run(self, *, plan_id: str, output_dir: str | Path) -> FeatureTable | None:
        ...
        target = output_dir / "eeg_features.csv"
        target.parent.mkdir(parents=True, exist_ok=True)
        frame.to_csv(target, index=False)
        return FeatureTable(modality="EEG", path=str(target), feature_columns=[c for c in frame.columns if c != "subject_id"], metadata={"source": str(source), "plan_id": plan_id})

    def _find_source(self) -> Path | None:
        explicit = self.config.get("features_csv")
        if explicit and Path(explicit).exists():
            return Path(explicit)
        fixture = Path("data/fixtures/toy_eeg_features.csv")
