...
def load_prompt(category: str, name: str, variables: dict[str, Any] | None = None) -> tuple[str, str, int]:
    prompt_path = Path(__file__).resolve().parents[1] / "hypothesis" / "prompts" / category / f"{name}.yaml"
...
class ChatLLMClient:
    """Shared OpenAI-compatible chat client with Ollama local-provider support."""
    ...
    def call(
        ...
        *,
        ...
        if bool(self.config.get("log_calls", True)):
            prompt_chars = sum(len(item.get("content", "")) for item in messages)
            log_prefix = str(self.config.get("log_prefix", "llm")).strip() or "llm"
            print(
                f"[{log_prefix}][llm] provider={self.provider} model={self.model} json={json_mode} messages={len(messages)} prompt_chars={prompt_chars}",
                flush=True,
            )
...
HypothesisLLMClient = ChatLLMClient
