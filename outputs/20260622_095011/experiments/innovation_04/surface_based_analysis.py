...
PARADIGM = {'id': 'surface_based_analysis',
 'name': 'Surface-based cortical analysis',
 'keywords': ['surface', 'fsaverage', 'fsnative', 'cortex', '皮层', '表面'],
 'required_data': ['surface_bold'],
 'outputs': ['surface_metrics', 'surface_figures'],
 'template': 'surface_analysis.py',
...
EXPERIMENT_PLAN = {'说明': '实验代码生成计划：从创新点/假设出发，明确可测量输出、分析步骤、风险控制，并在执行后用于反向更新创新点。',
 'paradigm_id': 'surface_based_analysis',
 'paradigm_name': 'Surface-based cortical analysis',
 'linked_innovations': [{'id': 'innovation_04',
                         'paradigm_id': 'surface_based_analysis',
                         'hypothesis': '皮层表面空间中的 BOLD 信号变化比体素空间更能保留皮层拓扑特征，可能揭示体积分析中被平滑掩盖的局部差异。',
                         'innovation_point': '提出“体积-表面一致性检查”：比较 surface BOLD 与 volume BOLD '
                                             '的可用性和信号摘要，识别哪些结论依赖空间表达方式。',
                         'experimental_plan': ['读取已处理 fMRI derivatives、QC 结果和可用 session。',
                                               '执行 surface_based_analysis 的类式 py 实验代码，提取可量化指标。',
                                               '检查指标是否符合预期结果模式，并记录关键图形或矩阵证据。',
                                               '如果执行失败或指标不支持假设，依据失败原因、替代解释和数据缺失情况反向修正创新点。'],
                         'testable_outputs': ['surface_figures', 'surface_metrics'],
                         'risk_factors': ['单被试结果不能直接推广到群体结论', '功能连接结果依赖 ROI 提取、配准质量和时间序列长度']}],
 'analysis_goal_zh': '用 Surface-based cortical analysis 的可执行输出检验关联创新点中的神经科学假设。',
 'measurable_outputs': ['surface_metrics', 'surface_figures'],
 'required_data': ['surface_bold'],
 'missing_data': [],
