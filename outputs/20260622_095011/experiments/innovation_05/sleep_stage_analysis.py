...
PARADIGM = {'id': 'sleep_stage_analysis',
 'name': 'Sleep-stage segmented analysis',
 'keywords': ['sleep', 'stage', 'nrem', 'rem', 'wake', '睡眠', '分期', '清醒'],
 'required_data': ['clean_bold', 'sleep_label'],
 'outputs': ['stagewise_metrics', 'stage_comparison'],
 'template': 'sleep_stage_analysis.py',
...
EXPERIMENT_PLAN = {'说明': '实验代码生成计划：从创新点/假设出发，明确可测量输出、分析步骤、风险控制，并在执行后用于反向更新创新点。',
 'paradigm_id': 'sleep_stage_analysis',
 'paradigm_name': 'Sleep-stage segmented analysis',
 'linked_innovations': [{'id': 'innovation_05',
                         'paradigm_id': 'sleep_stage_analysis',
                         'hypothesis': '睡眠/清醒片段之间的功能连接拓扑不同，清醒期更可能表现为跨网络连接增强，睡眠期更可能表现为局部或低频同步增强。',
                         'innovation_point': '提出“睡眠-清醒连接重排指数”：直接比较 W/S 分段 clean BOLD 的 top '
                                             'edges、平均连接强度和 3D connectome 空间分布。',
                         'experimental_plan': ['读取已处理 fMRI derivatives、QC 结果和可用 session。',
                                               '执行 sleep_stage_analysis 的类式 py 实验代码，提取可量化指标。',
                                               '检查指标是否符合预期结果模式，并记录关键图形或矩阵证据。',
                                               '如果执行失败或指标不支持假设，依据失败原因、替代解释和数据缺失情况反向修正创新点。'],
                         'testable_outputs': ['stage_comparison', 'stagewise_metrics'],
                         'risk_factors': ['单被试结果不能直接推广到群体结论', '功能连接结果依赖 ROI 提取、配准质量和时间序列长度']}],
 'analysis_goal_zh': '用 Sleep-stage segmented analysis 的可执行输出检验关联创新点中的神经科学假设。',
 'measurable_outputs': ['stagewise_metrics', 'stage_comparison'],
 'required_data': ['clean_bold', 'sleep_label'],
 'missing_data': [],
