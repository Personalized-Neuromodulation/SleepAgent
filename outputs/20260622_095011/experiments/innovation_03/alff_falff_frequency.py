...
PARADIGM = {'id': 'alff_falff_frequency',
 'name': 'ALFF/fALFF frequency-domain analysis',
 'keywords': ['alff', 'falff', 'frequency', '低频', '频域', '时频'],
 'required_data': ['clean_bold'],
 'outputs': ['alff_map', 'falff_map', 'summary_csv'],
 'template': 'alff_falff.py',
...
EXPERIMENT_PLAN = {'说明': '实验代码生成计划：从创新点/假设出发，明确可测量输出、分析步骤、风险控制，并在执行后用于反向更新创新点。',
 'paradigm_id': 'alff_falff_frequency',
 'paradigm_name': 'ALFF/fALFF frequency-domain analysis',
 'linked_innovations': [{'id': 'innovation_03',
                         'paradigm_id': 'alff_falff_frequency',
                         'hypothesis': '目标被试的低频 BOLD 振幅在部分 ROI 或体素簇中增强，可能对应局部自发活动强度差异。',
                         'innovation_point': '提出“低频振幅-连接耦合检验”：将 ALFF/fALFF 摘要与 ROI '
                                             '连接强度排序联合解释，区分局部振幅增强和网络连接增强。',
                         'experimental_plan': ['读取已处理 fMRI derivatives、QC 结果和可用 session。',
                                               '执行 alff_falff_frequency 的类式 py 实验代码，提取可量化指标。',
                                               '检查指标是否符合预期结果模式，并记录关键图形或矩阵证据。',
                                               '如果执行失败或指标不支持假设，依据失败原因、替代解释和数据缺失情况反向修正创新点。'],
                         'testable_outputs': ['alff_map', 'falff_map', 'summary_csv'],
                         'risk_factors': ['单被试结果不能直接推广到群体结论', '功能连接结果依赖 ROI 提取、配准质量和时间序列长度']}],
 'analysis_goal_zh': '用 ALFF/fALFF frequency-domain analysis 的可执行输出检验关联创新点中的神经科学假设。',
 'measurable_outputs': ['alff_map', 'falff_map', 'summary_csv'],
 'required_data': ['clean_bold'],
 'missing_data': [],
