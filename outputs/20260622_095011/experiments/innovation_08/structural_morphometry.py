...
PARADIGM = {'id': 'structural_morphometry',
 'name': 'Structural MRI morphometry',
 'keywords': ['structural',
              't1',
              'morphometry',
              'cortical thickness',
              ...
              '皮层厚度',
              '体积'],
 'required_data': ['structural_mri'],
 'outputs': ['structural_file_inventory', 'morphometry_plan'],
 'template': 'structural_morphometry.py',
...
EXPERIMENT_PLAN = {'说明': '实验代码生成计划：从创新点/假设出发，明确可测量输出、分析步骤、风险控制，并在执行后用于反向更新创新点。',
 'paradigm_id': 'structural_morphometry',
 'paradigm_name': 'Structural MRI morphometry',
 'linked_innovations': [{'id': 'innovation_08',
                         'paradigm_id': 'structural_morphometry',
                         'hypothesis': 'Structural MRI morphometry '
                                       '可以从当前数据中提取一个可量化的神经表型，但必须明确该表型对应的机制解释和失败条件。',
                         'innovation_point': '提出基于 Structural MRI morphometry '
                                             '输出的单被试机制检验：把主要指标、预期方向和替代解释同时纳入创新假设，而不是只报告范式结果。',
                         'experimental_plan': ['读取已处理 fMRI derivatives、QC 结果和可用 session。',
                                               '执行 structural_morphometry 的类式 py 实验代码，提取可量化指标。',
                                               '检查指标是否符合预期结果模式，并记录关键图形或矩阵证据。',
                                               '如果执行失败或指标不支持假设，依据失败原因、替代解释和数据缺失情况反向修正创新点。'],
                         'testable_outputs': ['morphometry_plan', 'structural_file_inventory'],
                         'risk_factors': ['单被试结果不能直接推广到群体结论', '功能连接结果依赖 ROI 提取、配准质量和时间序列长度']}],
 'analysis_goal_zh': '用 Structural MRI morphometry 的可执行输出检验关联创新点中的神经科学假设。',
 'measurable_outputs': ['structural_file_inventory', 'morphometry_plan'],
 'required_data': ['structural_mri'],
 'missing_data': [],
