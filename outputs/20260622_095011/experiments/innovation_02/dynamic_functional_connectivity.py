...
PARADIGM = {'id': 'dynamic_functional_connectivity',
 'name': 'Dynamic functional connectivity',
 'keywords': ['dynamic', 'sliding window', 'state', 'dfc', '动态', '状态'],
 'required_data': ['clean_bold'],
 'outputs': ['windowed_fc', 'state_features'],
 'template': 'dynamic_fc.py',
...
EXPERIMENT_PLAN = {'说明': '实验代码生成计划：从创新点/假设出发，明确可测量输出、分析步骤、风险控制，并在执行后用于反向更新创新点。',
 'paradigm_id': 'dynamic_functional_connectivity',
 'paradigm_name': 'Dynamic functional connectivity',
 'linked_innovations': [{'id': 'innovation_02',
                         'paradigm_id': 'dynamic_functional_connectivity',
                         'hypothesis': '目标被试的功能连接不是静态稳定的，而是在扫描过程中出现若干高连接和低连接窗口；窗口间连接矩阵变化幅度可以反映状态切换特征。',
                         'innovation_point': '提出“单被试动态连接不稳定性曲线”：用滑窗 FC 的均值强度、矩阵距离和 top-edge '
                                             '重排率刻画状态转移，而不是只给一个全程平均连接矩阵。',
                         'experimental_plan': ['读取已处理 fMRI derivatives、QC 结果和可用 session。',
                                               '执行 dynamic_functional_connectivity 的类式 py '
                                               '实验代码，提取可量化指标。',
                                               '检查指标是否符合预期结果模式，并记录关键图形或矩阵证据。',
                                               '如果执行失败或指标不支持假设，依据失败原因、替代解释和数据缺失情况反向修正创新点。'],
                         'testable_outputs': ['state_features', 'windowed_fc'],
                         'risk_factors': ['单被试结果不能直接推广到群体结论', '功能连接结果依赖 ROI 提取、配准质量和时间序列长度']}],
 'analysis_goal_zh': '用 Dynamic functional connectivity 的可执行输出检验关联创新点中的神经科学假设。',
 'measurable_outputs': ['windowed_fc', 'state_features'],
 'required_data': ['clean_bold'],
 'missing_data': [],
