...
PARADIGM = {'id': 'resting_state_functional_connectivity',
 'name': 'Resting-state functional connectivity',
 'keywords': ['resting',
              ...
              '功能连接',
              '网络'],
 'required_data': ['clean_bold'],
 'outputs': ['roi_correlation_matrix', 'fisher_z_matrix', 'network_summary'],
 'template': 'resting_state_fc.py',
 'prompt_match_score': 3,
 'data_compatibility_score': 20,
 'missing_data': [],
 'executable': True,
 'selection_score': 23}
EXPERIMENT_PLAN = {'说明': '实验代码生成计划：从创新点/假设出发，明确可测量输出、分析步骤、风险控制，并在执行后用于反向更新创新点。',
 'paradigm_id': 'resting_state_functional_connectivity',
 'paradigm_name': 'Resting-state functional connectivity',
 'linked_innovations': [{'id': 'innovation_01',
                         'paradigm_id': 'resting_state_functional_connectivity',
                         'hypothesis': '目标被试的静息态功能连接中，默认网络、感觉运动网络和小脑相关 ROI '
                                       '之间的连接强度会呈现可量化的不均衡模式；这种不均衡可作为单被试功能网络表型，而不是只报告平均相关矩阵。',
                         'innovation_point': '提出“跨网络-小脑连接偏置指数”：同时比较网络内连接、跨网络连接和小脑相关连接的 Fisher-Z '
                                             '强度，用一个方向性指标刻画单被试功能连接偏离模式。',
                         'experimental_plan': ['读取已处理 fMRI derivatives、QC 结果和可用 session。',
                                               '执行 resting_state_functional_connectivity 的类式 py '
                                               '实验代码，提取可量化指标。',
                                               '检查指标是否符合预期结果模式，并记录关键图形或矩阵证据。',
                                               '如果执行失败或指标不支持假设，依据失败原因、替代解释和数据缺失情况反向修正创新点。'],
                         'testable_outputs': ['fisher_z_matrix',
                                              'network_summary',
                                              'roi_correlation_matrix'],
                         'risk_factors': ['单被试结果不能直接推广到群体结论', '功能连接结果依赖 ROI 提取、配准质量和时间序列长度']}],
 'analysis_goal_zh': '用 Resting-state functional connectivity 的可执行输出检验关联创新点中的神经科学假设。',
 'measurable_outputs': ['roi_correlation_matrix', 'fisher_z_matrix', 'network_summary'],
 'required_data': ['clean_bold'],
 'missing_data': [],
...
def connectome_plot_coords(coords: np.ndarray, source_file: str) -> tuple[np.ndarray, str, str | None]:
    ...
    return transformed, "MNI152NLin2009cAsym", str(transform)
...
<section style="font-family: Arial, 'Microsoft YaHei', sans-serif; line-height: 1.6; padding: 16px 20px; border-bottom: 1px solid #ddd;">
  ...
  <p><strong>中文说明：</strong>本图使用 Nilearn <code>plotting.view_connectome</code> 绘制 3D 脑区功能连接。Nilearn 3D 头模使用 MNI/模板空间，因此节点绘图坐标使用 <strong>{coordinate_space}</strong> 空间{transform_note}；HTML 交互图中直接标注脑区名称，连接线表示 ROI 时间序列之间的 Pearson 相关系数。</p>
